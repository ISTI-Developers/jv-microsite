...
import { AccessAction, AccessMenu, fetchActionAccess, fetchJVUsers, fetchMenuAccess, saveUserActionAccess, saveUserMenuAccess } from '../action';
...
export default function JVUsersAccessPage() {
  ...
  const saveMenusMutation = useMutation({
    mutationFn: () => {
      if (!selectedUserId) {
        throw new Error('Please select a JV user first');
      }
    ...
    onSuccess: () => {
      toast.success('JV user menu access updated');
  ...
  const saveActionsMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('JV user action access updated');
    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h1 className="text-3xl font-semibold tracking-tight">JV Users Access</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Control menu and action overrides for a specific JV user.</p>
            </div>

            <Card className="w-full max-w-sm rounded-3xl bg-background/70 shadow-none">
              <CardContent className="p-5">
                <label className="text-sm font-medium">Select JV User</label>
      ...
      {!selectedUserId && (
        <Card className="rounded-3xl border-dashed shadow-sm">
          <CardContent className="p-8 text-center text-sm text-muted-foreground">Select a JV user first to manage access overrides.</CardContent>
        </Card>
      )}
          ...
          <Card className="rounded-3xl shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between gap-4">
              <div>
                ...
                <CardDescription>Set menu override access for this JV user.</CardDescription>
              </div>
          ...
          <Card className="rounded-3xl shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between gap-4">
              <div>
                ...
                <CardDescription>Set button/action overrides for this JV user.</CardDescription>
              </div>
