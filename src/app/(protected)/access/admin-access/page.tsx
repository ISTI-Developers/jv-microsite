...
export default function AdminAccessPage() {
  ...
  const { data: menus = [], isFetching: menusFetching } = useQuery<AccessMenu[]>({
    queryKey: ['access-menus', 'role', Roles.ADMIN],
    queryFn: () => fetchMenuAccess({ roleId: Roles.ADMIN }),
  });

  const { data: actions = [], isFetching: actionsFetching } = useQuery<AccessAction[]>({
    queryKey: ['access-actions', 'role', Roles.ADMIN],
    queryFn: () => fetchActionAccess({ roleId: Roles.ADMIN }),
  });
  ...
  const saveMenusMutation = useMutation({
    mutationFn: () => saveRoleMenuAccess(Roles.ADMIN, currentSelectedMenuIds),
    onSuccess: () => {
      toast.success('Admin menu access updated');
      queryClient.invalidateQueries({ queryKey: ['access-menus', 'role', Roles.ADMIN] });
  ...
  const saveActionsMutation = useMutation({
    mutationFn: () => saveRoleActionAccess(Roles.ADMIN, currentSelectedPermissionIds),
    onSuccess: () => {
      toast.success('Admin action access updated');
      queryClient.invalidateQueries({ queryKey: ['access-actions', 'role', Roles.ADMIN] });
    },
    onError: (err) => {
      if (err instanceof Error) {
        toast.error(err.message);
        ...
        <Card className="rounded-3xl shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>Select the menus that Admin users can see.</CardDescription>
            </div>
        ...
        <Card className="rounded-3xl shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>Select the buttons/actions that Admin users can use.</CardDescription>
            </div>
