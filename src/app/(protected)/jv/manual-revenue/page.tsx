...
import PageHeader from '@/app/(protected)/components/PageHeader';
import RevenueTable from '@/app/(protected)/unai-manual-revenue/RevenueTable';
import { Button } from '@/components/ui/button';
import { fetchJvMoaDetail, fetchJvMoaList, fetchJvRevenueAccountTitles, syncJvManualRevenue } from './action';
import {
  EditableRevenueItem,
  EditableRevenueMap,
  JvManualRevenueMoaListItem,
  JvManualRevenuePayload,
  RevenueRowValidationErrors,
  SelectedAccountsByLoc,
} from './types';
...
const moaColumns: Column<JvManualRevenueMoaListItem>[] = [
  {
    header: 'MOA',
    sortable: true,
...
const toUpsertPayload = ({ locId, accountKey, row }: RowPointer): JvManualRevenuePayload => {
  const payload: JvManualRevenuePayload = {
    location_id: row.location_id ?? locId,
...
function JvManualRevenuePageContent() {
  ...
  const moaListQuery = useQuery({
    queryKey: ['jv-moa-list-for-manual-revenue'],
    queryFn: fetchJvMoaList,
    enabled: !moaId,
  });

  const { data, isLoading, isFetching, isError, error, refetch } = useQuery({
    queryKey: ['jv-manual-revenue-moa-detail', moaId],
    queryFn: () => fetchJvMoaDetail(moaId),
    enabled: !!moaId,
  });
  ...
  const {
    ...
    queryKey: ['jv-manual-revenue-account-titles'],
    queryFn: fetchJvRevenueAccountTitles,
    enabled: !!moaId,
  });

  const initialRevenues = useMemo<EditableRevenueMap>(() => {
    if (!data?.jv_manual_revenue) return {};

    const mapped: EditableRevenueMap = {};

    Object.entries(data.jv_manual_revenue).forEach(([locId, accounts]) => {
      mapped[Number(locId)] = {};
  ...
  const currentRevenues = Object.keys(revenues).length > 0 || !data?.jv_manual_revenue ? revenues : initialRevenues;

  const currentSelectedAccountsByLoc =
    Object.keys(selectedAccountsByLoc).length > 0 || !data?.jv_manual_revenue ? selectedAccountsByLoc : initialSelectedAccountsByLoc;
  ...
  const mutation = useMutation({
    mutationFn: () => syncJvManualRevenue(moaId, upserts, deletePayload),
    onSuccess: async () => {
      toast.success('JV manual revenue saved successfully');
      setRevenues({});
  ...
  if (!moaId) {
    return (
      <div className="space-y-6">
        <PageHeader title="JV Manual Revenue" subtitle="Select an MOA to encode JV manual revenue entries." icon={HandCoins} />
        ...
        <div className="mt-4">
          {moaListQuery.isLoading ? (
            ...
            <div className="rounded-2xl border border-dashed border-border bg-background/70 px-4 py-3 text-sm text-muted-foreground">
              Open this page with a MOA query parameter, for example <span className="font-medium">/jv/manual-revenue?moa_id=123</span>.
            </div>
            ...
            <DataTable
              ...
              onRowClick={(row) => router.push(`/jv/manual-revenue?moa_id=${row.id}`)}
            />
          ) : (
            <div className="rounded-2xl border border-dashed border-border bg-background/70 px-4 py-3 text-sm text-muted-foreground">
              No MOAs found. Open this page with a MOA query parameter, for example <span className="font-medium">/jv/manual-revenue?moa_id=123</span>
              .
            </div>
          )}
        </div>
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <PageHeader
          title="JV Manual Revenue"
          subtitle="Loading MOA revenue details"
          icon={HandCoins}
          actions={
            <Button variant="outline" onClick={() => router.push('/jv/manual-revenue')}>
              Back
            </Button>
          }
        />
          ...
          <>
            ...
            {(showSaveActions || mutation.isPending) && (
              <Button onClick={handleSave} disabled={mutation.isPending || isFetching || !hasRevenueChanges} className="h-10 rounded-xl px-5">
                {mutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : 'Save JV Manual Revenue'}
              </Button>
            )}
          </>
        }
              ...
              selectedCats.map((cat) => {
                ...
                return (
                  <RevenueTable
                    ...
                    emptyTitle="No rows yet. Use Add Row to start entering JV manual revenue for this account."
                  />
                );
              })
            ) : (
            ...
            <div className="sticky bottom-4 z-10 rounded-3xl border border-border bg-card/95 p-4 shadow-lg backdrop-blur">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3 text-sm">
                  ...
                  <div>
                    <p className="font-medium text-foreground">JV manual revenue changes</p>
...
export default function JvManualRevenuePage() {
  return (
    <Suspense
      fallback={
        <div className="rounded-3xl border border-border bg-card p-8 text-sm text-muted-foreground shadow-sm">
          <div className="flex items-center gap-3">
            <LoaderCircle className="size-5 animate-spin" />
            Loading JV manual revenue page...
          </div>
        </div>
      }
    >
      <JvManualRevenuePageContent />
    </Suspense>
  );
}
