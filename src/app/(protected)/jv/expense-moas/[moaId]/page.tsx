...
type MoaData = {
  ...
  locations: Location[];
  categories: Category[];
  expenses: Record<number, ExpenseItem[]>;
};
...
export default function JVExpenseMoaDetailPage() {
  ...
  const mergedExpenses =
    Object.keys(expenses).length === 0 && data?.expenses ? data.expenses : expenses;
  return (
    <Box>
      ...
      {data?.categories.map((cat) => (
        <Box key={cat.id} mb={2}>
          ...
          {(mergedExpenses[cat.id] || []).map((exp, i) => (
            <Box key={exp.id ?? i} ml={2}>
              {exp.other_label ? `${exp.other_label}` : exp.name} - {exp.amount} - {exp.date}
            </Box>
          ))}
        </Box>
      ))}
