...
function DialogOverlay({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Overlay>) {
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        'fixed inset-0 z-40 bg-black/55 backdrop-blur-sm duration-100 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0',
        className
      )}
      {...props}
    />
...
function DialogContent({
  ...
  return (
    <DialogPortal>
      {showOverlay && (
        <div
          data-slot="dialog-custom-overlay"
          className={cn(
            'fixed inset-0 z-40 bg-black/55 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
            overlayClassName
          )}
